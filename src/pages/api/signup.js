...
const CONTACT_MESSAGE_FIELDS = {
    ...
    address: "Address",
    contactTime: "Best time to communicate",
    method: "Perferred Contact",
    email: "Email",
    message: "Message",
};

const genrateEmailContent = (data) => {
...
const handler = async (req, res) => {
    ...
    {
        const data = req.body;
        if( !data.name || !data.phoneNumber || !data.address || !data.contactTime || !data.method || !data.email || !data.message) {
            return res.status(400).json({message: "Bad request"});
        }
        try{
            await transporter.sendMail({
                ...mailOptions,
                ...genrateEmailContent(data),
                subject: data.name,
            });
